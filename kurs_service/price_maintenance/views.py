...
def price_maintenance(request, category_slug=None):
	if category_slug:
		category = PriceMaintenanceCategory.objects.filter(
			slug=category_slug
		).first()
		price_list = PriceMaintenancePrice.objects.filter(category=category)
	else:
		category = None
